...
export function ChannelMutateDrawer({
  ...
  const currentCostEnabled = formValues.cost_enabled
  const currentCostMode = formValues.cost_mode
  const currentCostModelPrices = formValues.cost_model_prices
  const {
  ...
  const costFields = (
    <ChannelCostSection>
      <div
        role='group'
        aria-label={t('Call Cost')}
        className={channelConfigurationBlockClassName(
          configuration.blocks.costAccounting,
          'space-y-4'
        )}
      >
        <fieldset
          disabled={sensitiveLocked}
          className='space-y-0 disabled:opacity-60'
        >
          <FormField
            control={form.control}
            name='cost_enabled'
          ...
          <>
            ...
            {currentCostMode === 'discount' ? (
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('Discount Ratio')}</FormLabel>
                    <FormControl>
                      <Input
                        type='number'
                        step='0.01'
            ...
            {currentCostMode === 'discount' && (
              <div className='space-y-3'>
                ...
                <ChannelCostPriceTable
                  ...
                  onChange={(prices) =>
                    form.setValue('cost_model_prices', prices, {
                      shouldDirty: true,
                    })
                  }
                />
